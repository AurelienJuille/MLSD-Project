...
def lolffate_pipeline():
    ...
    preprocessing_task = preprocessing(dataset=ingestion_task.outputs["dataset"])
    ...
    training_task = training(
        preprocessed_dataset_train=train_test_split_task.outputs[
    ...
    evaluation_task = evaluation(
        ...
        preprocessed_dataset_test=train_test_split_task.outputs[
            "preprocessed_dataset_test"
        ],
    )

    deploy_model_task = deploy_model(model=training_task.outputs["model"])


def main():
    compiler.Compiler().compile(
        pipeline_func=lolffate_pipeline, package_path="lolffate_pipeline.json"
    )
